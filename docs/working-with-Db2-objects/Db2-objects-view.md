...
2. Expand the **Views** section and click the view name. It displays the following view details:
   ...
   **Data**: Shows the preview of the actual data returned by the view.
   ...
   >**Note**: The **Max rows** specifies the maximum number of rows to show in the data preview. Enter the required value in the **Max rows** and press **Enter** to refresh the preview. You can also set the row limit by using the **Db2service: Max Rows** extension setting. See Liming the number of rows returned for more information. The **Max rows** field is synchronized with the **Db2service: Max Rows** extension setting, and changes made in either location automatically update both.
   {: .note-right}
