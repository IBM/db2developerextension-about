...
- `db2-for-luw-1.2.0.vsix` - Extension package  
- `db2-for-luw-1.2.0.vsix.cosign.sig` - Digital signature  
...
base64 -D -i db2-for-luw-1.2.0.vsix.cosign.sig -o db2-for-luw-1.2.0.vsix.sig

# Verify with Cosign
cosign verify-blob \
  --key PRD0015359key.pem.pub.key \
  --signature db2-for-luw-1.2.0.vsix.sig \
  --insecure-ignore-tlog=true \
  db2-for-luw-1.2.0.vsix
```

**Linux:**
```bash
# Decode the signature
base64 -d db2-for-luw-1.2.0.vsix.cosign.sig > db2-for-luw-1.2.0.vsix.sig
...
```powershell
# Decode the signature
[System.Convert]::FromBase64String((Get-Content db2-for-luw-1.2.0.vsix.cosign.sig)) | Set-Content -Encoding Byte db2-for-luw-1.2.0.vsix.sig
